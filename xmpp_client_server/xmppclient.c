...
int BenchMark = 1000;           // Chage as per your requirement
int PAYLOAD_SIZE;
...
int main(int argc, char *argv[])
{
    ...
    if(Connect_TCP_Sock() != 0)
    {
        ...
        // printf("%sEnter any key to continue...\n", KWHT);
        // getchar();
...
void Publish()
{
    ...
    sprintf(message, "<iq type='set' to='pubsub.%s' from='%s' id='%d'><publish item='%s'><payload>%s</payload></publish></iq>", remotehost, client, (msg_sequence_number+1), p_stanza.item, payload);
    TCP_SendTo();
